...
const modalButton=()=>{
        ...
        for (var i = 0; i < button.length; i++) {    
            ...
            return function() {
                ...
                conteudo=`<div id="delModal"><i onclick="fecharModal() "class="fas fa-times-circle"></i><i onclick="printDiv('imprimir')"class="fas fa-times-circle"></i></div><div id="imprimir" style="color:black;"onclick="modalButton()"><h1>${people[j].nome}</h1><h3>${people[j].sub}</h3><hr><div class="subContent" id="subContent"><p>Cargo: ${people[j].job}</p><p>Aniversario: ${people[j].birth}</p><p>Tel: ${people[j].phone}</p><p>Bairro: ${people[j].bairro}</p><p>Rua: ${people[j].endereco}</p><p>№: ${people[j].num}</p></div><div id="id"></div></div>`
